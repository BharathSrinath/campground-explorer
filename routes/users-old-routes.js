...
router.post('/register', catchAsync(async (req, res, next) => {
    try {
        ...
        req.login(registeredUser, err => {
            ...
            req.flash('success', 'Welcome to Destination Diaries!');
...
router.post('/login', storeReturnTo, passport.authenticate('local', { failureFlash: true, failureRedirect: '/login' }), (req, res) => {
    ...
    // If we try to edit a destination without being logged-in, we will be redirected to login page. So whenever we are redirected to a login page, returnTo will have a value. 
    // But when a user directly clicks the login, the authentication check (in middlware.js) will not happen in the first place. So in this scenario, returnTo will be empty. In that scenario, we are sending them to /destinations.
